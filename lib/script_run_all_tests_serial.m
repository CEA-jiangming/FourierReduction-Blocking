...
if run_pdfb_bpcon_par_sim_rescaled
    ...
    for i = 1:num_tests
        ...
        [result_st.sol{i}, result_st.L1_v{i}, result_st.L1_vp{i}, result_st.L2_v{i}, ...
            result_st.L2_vp{i}, result_st.delta_v{i}, result_st.sol_v{i}, result_st.snr_v{i}, result_st.time_v{i}, ~, ~, result_st.sol_reweight_v{i}] ...
...
%% rescaled PD with simulated parallelism over singular values

% if run_pdfb_bpcon_par_sing_sim_rescaled
%     
%     result_st = [];
%     result_st.sol = cell(num_tests, 1);
%     result_st.L1_v = cell(num_tests, 1);
...
% %     result_st.param_fouRed = param_fouRed;
% 
%      
%     for i = 1:num_tests
%         % wavelet mode is a global variable which does not get transfered
...
%         [result_st.sol{i}, result_st.L1_v{i}, result_st.L1_vp{i}, result_st.L2_v{i}, ...
%             result_st.L2_vp{i}, result_st.delta_v{i}, result_st.sol_v{i}, result_st.snr_v{i}, ~, ~, result_st.sol_reweight_v{i}] ...
%             = pdfb_bpcon_par_sing_sim_rescaled_adapt_eps(yT{i}, [Ny, Nx], epsilonT{i}, epsilonTs{i}, epsilon{i}, epsilons{i}, FIpsf, FIpsf_t, T, W, Psi, Psit, Psiw, Psitw, param_pdfb);
%         tend = toc(tstart_a);
...
if run_pdfb_bpcon_par_sim_rescaled_precond
    ...
    for i = 1:num_tests
        ...
        [result_st.sol{i}, result_st.L1_v{i}, result_st.L1_vp{i}, result_st.L2_v{i}, ...
            result_st.L2_vp{i}, result_st.delta_v{i}, result_st.sol_v{i}, result_st.snr_v{i}, result_st.time_v{i}, result_st.no_sub_itr_v{i}, ~, ~, ...
            result_st.sol_best_bound_v{i}, result_st.sol_reweight_v{i}] ...
            = pdfb_bpcon_par_sim_rescaled_precond(yT{i}, epsilonT{i}, epsilonTs{i}, epsilon{i}, epsilons{i}, A, At, T, aW, W, Psi, Psit, Psiw, Psitw, param_pdfb);
        tend = toc(tstart_a);
...
%% rescaled PD with simulated parallelism over singular values and preconditioning
if run_pdfb_bpcon_par_sing_sim_rescaled_precond
...
%     result_st.param_fouRed = param_fouRed;
    ...
    for i = 1:num_tests
        ...
        [result_st.sol{i}, result_st.L1_v{i}, result_st.L1_vp{i}, result_st.L2_v{i}, ...
            ...
            = pdfb_bpcon_par_sing_sim_rescaled_precond(yT{i}, [Ny, Nx], epsilonT{i}, epsilonTs{i}, epsilon{i}, epsilons{i}, FIpsf, FIpsf_t, T, aW, W, Psi, Psit, Psiw, Psitw, param_pdfb);        
        tend = toc(tstart_a);
        fprintf(' pdfb_bpcon_par_sim_rescaled_precond runtime: %ds\n\n', ceil(tend));
    ...
    results_prefix = 'pdfb_bpcon_par_sing_sim_rescaled_precond';
    param_structure_name = 'param_pdfb';
