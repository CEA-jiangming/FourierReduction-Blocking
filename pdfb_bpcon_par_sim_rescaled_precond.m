...
best_bound_alpha_ff = param.best_bound_alpha_ff;
best_bound_alpha = param.best_bound_alpha;
...
%             vy2{q} = v2{q} + pU{q} .* r2{q} - (pU{q} .* 1 ./ sqrt(pU{q})) .* proj{q};
    ...
    if reweight_step_count_since_best_bound_search >= param.best_bound_reweight_steps && ...
            ((param.use_best_bound_steps && t == best_bound_steps(best_bound_step_count)) || ...
            (param.use_best_bound_eps && ...
            norm(cell2mat(norm2)) <= norm(cell2mat(epsilonts)) && ...
            rel_sol_norm_change < param.best_bound_rel_obj))
        
        for q = 1:R
            epsilont{q} = best_bound_alpha_ff * epsilont{q};
            epsilonts{q} = best_bound_alpha * epsilont{q};
        end
        
        fprintf('\n\n\n\n\n\n\n Decreased epsilon - %d time \n\n\n\n\n', best_bound_step_count);

        sol_best_bound_v(best_bound_step_count, :, :) = xsol;                
        best_bound_step_count = best_bound_step_count + 1;
        
        reweight_step_count_since_best_bound_search = 0;
        reweight_alpha = param.reweight_alpha;
        
        if norm(param.best_bound_stop_eps_v) > norm(cell2mat(epsilont))
            break;
        end
    end
