function [SNR, DR, M, histogrampeakiness, tend] = rsing_discardsingvalues_admm(visibSize, input_snr, imgfile, coveragefile, klargestpercent, run)
% inputs
...
fprintf('Computing holographic matrix and embedded data vector... ');
tstart1=tic;
h = (st.p)'*(st.p);
ht = h';
if(diagholographic)
...
if(~Alldatafromfiles)
    ...
    xo = 0; yo = 0;
    y4 = fft2(conj(st.sn).*y3(yo+1:yo+Ny,xo+1:xo+Nx)); % Cropping and 4. scaling done in one step
    y4 = y4(:);
else
    fprintf('Loading y from saved file...');
    load(yfile);
    y4 = y; clear y;
end
fprintf('Done\n');
...
serialise = @(x) x(:);
A = @(x) Mask*serialise(fft2(reshape(grid2imgsize_fwd(x, st, ht, 1), st.Nd(1), st.Nd(2))));
At = @(x) grid2imgsize_adj(serialise(ifft2(reshape(Mask'*x, st.Nd(1), st.Nd(2)))), st, ht, 1);
